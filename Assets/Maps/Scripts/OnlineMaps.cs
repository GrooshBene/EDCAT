...
public class OnlineMaps : MonoBehaviour
{
    ...
    public OnlineMapsMarker AddMarkerCat(Vector2 markerPosition, Texture2D markerTexture = null, string label = "",int id = 0)
    {
        if (markerTexture == null) markerTexture = defaultMarkerTexture;
        ...
        OnlineMapsMarker marker = new OnlineMapsMarker
        {
            ...
            align = defaultMarkerAlign,
            cated = true,
            cat = new Cat(CatDatabase.instance.FindCat(id))
    };
        marker.Init();
        ms.Add(marker);
        markers = ms.ToArray();
    ...
    public OnlineMapsMarker AddMarkerHotSpot(Vector2 markerPosition, Texture2D markerTexture = null, string label = "",int id = 0)
    {
        if (markerTexture == null) markerTexture = defaultMarkerTexture;
        ...
        OnlineMapsMarker marker = new OnlineMapsMarker
        {
            ...
            align = defaultMarkerAlign,
            hotspoted = true,
            hotspot = HotSpotDatabase.instance.Find(id)
        };
        marker.Init();
        ms.Add(marker);
        markers = ms.ToArray();
        ...
        if (!string.IsNullOrEmpty(tooltip))
        {
            if (tooltipMarker != null)
            {
                ...
                if (tooltipMarker.OnDrawTooltip != null) { tooltipMarker.OnDrawTooltip(tooltipMarker);
                    ...
                    if (tooltipMarker.hotspoted && UserData.instance.HotspotInfo.activeSelf == false && UserData.instance.UIopen == false)
                    {
                        tooltipMarker.OpenHotSpot();
                    }
                    else if (tooltipMarker.cated && UserData.instance.UIopen == false) {
                        tooltipMarker.OpenCatAR();
                    }
                }
            }
