...
public class OnlineMapsMarkerBase
{
    ...
    public void OpenHotSpot() {
        HotSpotInfo info = UserData.instance.HotspotInfo.GetComponent<HotSpotInfo>();
        info.h = hotspot;
        UserData.instance.HotspotInfo.SetActive(true);
        UserData.instance.HotspotInfo.GetComponent<HotSpotInfo>().SetData();
    }

    public void OpenCatAR()
    {
        GameManager.instance.AR = cat;
        SceneManager.LoadScene("PlayAR");
        Debug.Log("qqq");
    }
