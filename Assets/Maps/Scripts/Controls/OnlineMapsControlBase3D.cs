...
public class OnlineMapsControlBase3D : OnlineMapsControlBase
{
    ...
    protected override void BeforeUpdate()
    {
        ...
        if (allowCameraControl)
        {
            ...
            if (Input.GetMouseButton(1))
            {
                ...
                cameraRotation.x = Mathf.Clamp(cameraRotation.x, -1, 1); 
                lastMousePosition = mousePosition;
                
                print(activeCamera.transform.parent);
            ...
            if (!allowZoom && allowCameraControl)
            {
                if (Input.touchCount == 1)
                {
                    ...
                    Vector2 p2 = Input.GetTouch(0).position;

                    Vector2 center = Vector2.Lerp(p2, p2, 0.5f);
    ...
    private void UpdateCameraPosition()
    {
        ...
        activeCamera.transform.RotateAround(activeCamera.transform.parent.position, Vector3.up, cameraSpeed.x * cameraRotation.x * Time.deltaTime);

        Vector3 newPosition = activeCamera.transform.position;
        */
